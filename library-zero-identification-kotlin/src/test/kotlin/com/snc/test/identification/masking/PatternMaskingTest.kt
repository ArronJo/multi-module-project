...
class PatternMaskingTest : BaseJUnit5Test() {
    ...
    fun `패턴 마스킹 - 주소 상세주소`() {
        ...
        assertEquals(v1, "서울특별시 강남구 테헤란로 123 길동**** 5*")

        val v2 = PatternMasking.addressDetail("경기도 성남시 분당구 판교동 123-45 판교아파트 101동 1001호")
        logger.debug { v2 }     // 출력: 경기도 성남시 분당구 판교동 123-45 판교***** 10***
        assertEquals(v2, "경기도 성남시 분당구 판교동 123-45 판교***** 10***")

        val v3 = PatternMasking.addressDetail("제주특별자치도 제주시 연동 1234")
        logger.debug { v3 }      // 출력: 제주특별자치도 제주시 연동 1234
        assertEquals(v3, "제주특별자치도 제주시 연동 1234")

        val v4 = PatternMasking.addressDetail("서울시 송파구 잠실동 123-456 롯데타워 123층")
        logger.debug { v4 }       // 출력: 서울시 송파구 잠실동 123-456 롯데*** 12**
        assertEquals(v4, "서울시 송파구 잠실동 123-456 롯데*** 12**")

        val v5 = PatternMasking.addressDetail("서울시 강남구 역삼동 123-45 A동")
        logger.debug { v5 }       // 출력: 서울시 강남구 역삼동 123-45 A*
        assertEquals(v5, "서울시 강남구 역삼동 123-45 A*")

        val v6 = PatternMasking.addressDetail("A동 1004호")
        logger.debug { v6 }       // 출력: A* 10**
        assertEquals(v6, "A* 10**")
    }
}