...
class HangulMatcher {
    ...
    private fun matchByJamo(target: String, startIndex: Int): HangulMatchResult {
        ...
        for (searchPos in startIndex until target.length) {
            val result = tryMatchAtPosition(target, searchPos, keywordDecomposed)
            if (result != HangulMatchResult.EMPTY) {
                return result
            }
        }
    ...
    // 특정 위치에서 매칭 시도
    private fun tryMatchAtPosition(target: String, searchPos: Int, keywordDecomposed: String): HangulMatchResult {
        var decomposedStr = ""
